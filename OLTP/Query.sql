...
CREATE TABLE creditos (
    ...
    CONSTRAINT chk_creditos_valor_cuota CHECK (valor_cuota > 0),
    CONSTRAINT chk_creditos_desgravamen CHECK (desgravamen >= 0)
);
...
CREATE TABLE cronograma_cuotas (
    ...
    CONSTRAINT chk_num_cuota            CHECK (num_cuota > 0),
    CONSTRAINT chk_tasa_mora            CHECK (tasa_mora >= 0)
);
...
CREATE TABLE detalle_cuotas_pagos (
    ...
    CONSTRAINT fk_detalle_cuota         FOREIGN KEY (id_cuota) REFERENCES cronograma_cuotas(id),
...
CREATE TABLE alertas_mora (
    ...
    CONSTRAINT fk_alertas_cuota     FOREIGN KEY (id_cuota)   REFERENCES cronograma_cuotas(id)
);
...
ALTER TABLE personas_naturales
ADD CHECK (estado_civil IN ('S', 'C', 'D', 'V', 'U'));
...
ALTER TABLE personas_juridicas
ADD CHECK (inicio_actividades >= fecha_constitucion);
...
ALTER TABLE cuentas_bancarias
ADD CHECK (fecha_vencimiento IS NULL OR fecha_vencimiento > fecha_apertura);
...
ALTER TABLE productos_crediticios
ADD CHECK (deleted_at IS NULL OR estado = 'inactivo');
...
ALTER TABLE solicitudes
ADD CHECK (estado != 'desestimado' OR motivo_rechazo IS NOT NULL);

ALTER TABLE solicitudes
ADD CHECK (estado NOT IN ('aprobada', 'desestimado') OR fecha_resolucion IS NOT NULL);
...
ALTER TABLE evaluaciones_riesgo
ADD CHECK (ingresos_verificados > 0);
 
ALTER TABLE evaluaciones_riesgo
ADD CHECK (deuda_activa >= 0);
 
ALTER TABLE evaluaciones_riesgo
ADD CHECK (creditos_activos >= 0);
...
ALTER TABLE alertas_mora
ADD CHECK (
    tipo_alerta NOT IN ('cuota_vencida', 'mora_30d', 'mora_60d', 'mora_90d')
    OR id_cuota IS NOT NULL
);

ALTER TABLE alertas_mora
ADD CHECK (fecha_cierre IS NULL OR fecha_cierre >= fecha_alerta);

ALTER TABLE alertas_mora
ADD CHECK (estado != 'cerrada' OR fecha_cierre IS NOT NULL);
